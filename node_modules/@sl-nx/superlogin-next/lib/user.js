...
})(ValidErr = exports.ValidErr || (exports.ValidErr = {}));
class User {
    constructor(config, userDB, couchAuthDB, mailer, emitter, couchServer) {
        ...
        this.onLinkActions = [];
        this.hasher = new hashing_1.Hashing(config);
        this.session = new session_1.Session(this.hasher);
        ...
        for (const [k, v] of Object.entries((_a = config.local.consents) !== null && _a !== void 0 ? _a : {})) {
            if (v.required) {
    ...
    async prepareNewUser(newUser) {
        ...
        newUser.local = await this.hashPassword((_a = newUser.password) !== null && _a !== void 0 ? _a : (0, util_1.URLSafeUUID)());
    ...
    async insertNewUserDocument(newUser, req) {
        ...
        if (this.config.local.sendConfirmEmail) {
            await this.mailer.sendEmail('confirmEmail', newUser.unverifiedEmail.email, {
                req: req,
                user: newUser
            });
        }
    ...
    async createSession(login, provider, byUUID = false) {
        ...
        const password = 'cazzominchia'; //// MOD

        token.provider = provider;
        await this.dbAuth.storeKey(user.key, (0, util_1.hyphenizeUUID)(user_uid), token.key, password, token.expires, user.roles, provider);
    ...
    async logoutUserSessions(userDoc, op, currentSession) {
        ...
        if (op === Cleanup.all || op === Cleanup.other) {
            sessions = (0, util_1.getSessions)(userDoc);
        }
        else if (op === Cleanup.expired) {
            sessions = (0, util_1.getExpiredSessions)(userDoc, Date.now());
        }
    ...
    async confirmSession(key, password) {
        try {
            ...
            if (doc.expires > Date.now()) {
                ...
                return { key, ...(await this.session.confirmToken(doc, password)) };
            }
            else {
        ...
        const processUserDBs = (dbList, type) => {
            dbList.forEach(userDBName => {
                ...
                promises.push(this.dbAuth
                    .addUserDB(userDoc, userDBName, dbConfig.designDocs, type, dbConfig.adminRoles, dbConfig.memberRoles)
                    .then(finalDBName => {
                    delete dbConfig.adminRoles;
                    delete dbConfig.memberRoles;
                    delete dbConfig.designDocs;
     ...
     */
    async getCurrentConsents(login) {
        const userDoc = await this.userDbManager.getUser(login, true);
        return (0, util_1.extractCurrentConsents)(userDoc);
    }
    /**
     * @internal
     ...
     */
    async updateConsents(login, consentUpdate) {
        const validationErrMsg = (0, util_1.verifyConsentUpdate)(consentUpdate, this.config);
        if (validationErrMsg) {
            throw { error: 'bad request', status: 400, message: validationErrMsg };
        }
        let userDoc = await this.userDbManager.getUser(login, true);
